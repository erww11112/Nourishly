        ...
        {tab==="profile"&&(
          <div>
            ...
            <div style={{ background:C.card, borderRadius:R.xl, border:`1px solid ${C.border}`, boxShadow:SHADOW.card, padding:"22px", marginBottom:20 }}>
              ...
              {/* Language pills — grid of 2-3 per row so all 10 languages fit without overflowing on mobile */}
              <div style={{ display:"flex", flexWrap:"wrap", gap:8, background:C.bg, borderRadius:R.md, padding:8, direction:"ltr" }}>
                {[["en",t("profile.languageEnglish")],["pt",t("profile.languagePortuguese")],["es",t("profile.languageSpanish")],["zh",t("profile.languageChinese")],["fr",t("profile.languageFrench")],["de",t("profile.languageGerman")],["it",t("profile.languageItalian")],["ru",t("profile.languageRussian")],["hi",t("profile.languageHindi")],["ar",t("profile.languageArabic")]].map(([code,label])=>(
                  <button key={code} onClick={()=>handleChangeLanguage(code)} className="btn-press" style={{ flex:"1 1 calc(33.333% - 6px)", minWidth:90, padding:"9px 6px", borderRadius:R.sm, border:"none", cursor:"pointer", fontWeight:700, fontSize:13, fontFamily:"inherit", background:lang===code?C.card:"transparent", color:lang===code?C.walnut:C.muted, boxShadow:lang===code?"0 2px 8px rgba(44,24,16,0.1)":"none", transition:"all 0.15s", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis", textAlign:"center" }}>{label}</button>
                ))}
              </div>
            </div>
